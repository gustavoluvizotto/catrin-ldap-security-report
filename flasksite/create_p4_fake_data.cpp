...
struct switch_t {
    ...
    uint8_t as_manrs_member:1; // indicates if AS is participating in MANRS (best routing practices)
    uint8_t as_mb_risk:2; // middlebox risk level
    uint8_t padding:1;
};
#pragma pack(pop)
...
int main(int argc, char** argv) {
    ...
    // scenario 1: up path has high MB risk but participate in MANRS and has ROV score
    // scenario 2: low path has low MB risk but does not participate in MANRS and has no ROV score

    // AS 1; entry AS; EU; support ROV, MANRS and low MB risk
    h.kind = 0x72;
    h.length = 8;
    h.as_number = 65'537; // 65'536 - 65'551
    h.as_country = 4;
    h.as_type = 1;
    h.as_rov_score = 1; // 0 or 1 (1 for 100% ROV, otherwise 0)
    h.as_manrs_member = 1; // participating in MANRS
    h.as_mb_risk = 0; // 0-3 scale;
